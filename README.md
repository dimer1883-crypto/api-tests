...
![CI](https://github.com/dimer1883-crypto/api-tests/actions/workflows/tests.yml/badge.svg)
...
не хардкодится: тест читает его через `os.environ`. С заданным токеном весь набор
проходит целиком (12 тестов).

## CI

Workflow `.github/workflows/tests.yml`: на каждый push и pull request запускается
Ubuntu-раннер, ставится Python 3.13, устанавливаются зависимости из `requirements.txt`
и выполняется `pytest`. Три теста GitHub API в CI пропускаются (токен там не задан),
остальные 9 проходят.
